...
//		    console.log(detail)
//		    console.log(detail.exec_info.queue_remaining)
            //弹出提示框，当前队列中
            if(detail.exec_info.queue_remaining>0){
                this.validate.showNotifyWarnRight("The current queue is in progress, please wait...");
//                console.log($(".comfy-queue-btn"))
                $($(".comfy-queue-btn")[0]).css("cursor","not-allowed");//更改鼠标光标样式禁用
                $($(".comfy-queue-btn")[0]).css("opacity","0.5");//降低按钮不透明度
                // 禁用按钮点击事件
                $($(".comfy-queue-btn")[0]).prop("disabled", true);
                ComfyApp.execute_task=true;
            }else{
            	//暂停5000毫秒恢复按钮
            	if(ComfyApp.execute_task==true){
            	    ComfyApp.execute_task=false;
            	    this.validate.showNotifySuccessLeft("The queue has ended and will be available in 5 seconds. Please wait...");
                    let interval=setInterval(function(){
                        clearInterval(interval);
                        $($(".comfy-queue-btn")[0]).css("cursor","pointer");
                        $($(".comfy-queue-btn")[0]).css("opacity","1");
                        // 在事件执行完后重新启用按钮
                        $($(".comfy-queue-btn")[0]).prop("disabled", false);
                     },5000);
            	}else{
                    $($(".comfy-queue-btn")[0]).css("cursor","pointer");
                    $($(".comfy-queue-btn")[0]).css("opacity","1");
